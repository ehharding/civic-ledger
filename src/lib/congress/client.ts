 ...
 * |--------------------------|-----------------------------------------------------------------------------| |
 * `api-schema.ts` | Runtime shapes for Congress.gov v3 payloads — the untrusted-input boundary. | | `http.ts` | Key
 * access, URL building, caching policy, one request helper, route guards. | | `mappers.ts` | Upstream shapes to this
 * app's stable model. Pure; no I/O. | | `bills.ts` | Bill snapshots, pagination, lookup, summaries, text versions,
 * search. | | `composition.ts` | Chamber membership, including the member list's pagination. | |
 * `committee-directory.ts` | Every committee of a Congress, reshaped into one browsable list. | | `committee-filter.ts`
 * | The committee directory's narrowing, ordering, and URL rules. Pure; no I/O. | | `committee-profile.ts` | One
 * committee's record, its history, and its subcommittees. | | `member-directory.ts` | The same membership, reshaped
 * into one browsable alphabetical roster. | | `member-filter.ts` | The directory's narrowing, ordering, and URL rules.
 * Pure; no I/O. | | `member-profile.ts` | One member's record, plus the legislation they sponsored and cosponsored. |
 *

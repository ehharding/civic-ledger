...
import { requestBillDetail } from "@/lib/congress/bills/sub-resource";
...
import { previewCommitteeRecords } from "@/lib/congress/upstream/fixtures";
import {
  buildCongressUrl,
 ...
 *
 * The I/O half of the pair whose model and URL rules live in `records.ts`, on exactly the split
 * `directory.ts`/`filter.ts` already draws. It holds the adapter's two standing invariants: nothing throws, and

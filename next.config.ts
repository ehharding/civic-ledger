...
const nextConfig: NextConfig = {
  ...
  // host distinct from localhost and refuses to serve dev-only assets to. Nothing in the suite depends on those
  // assets, so the run passed anyway — it just printed a block warning per page load, which is exactly the kind of
  // routine noise that trains you to stop reading e2e output. Development-only; the setting has no production effect.
